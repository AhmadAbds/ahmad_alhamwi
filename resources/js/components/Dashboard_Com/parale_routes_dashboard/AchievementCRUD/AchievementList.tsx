...
interface AchieListProps {
  onEdit: (achievement: AchievData) => void;
}

export default function AchieveList({ onEdit }: AchieListProps) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await achievementsService.deleteachievements(id);
      setachievements(achievements.filter(ele => ele.id !== id));
    } catch (err) {
      setError('فشل في حذف');
      console.error(err);
    }
