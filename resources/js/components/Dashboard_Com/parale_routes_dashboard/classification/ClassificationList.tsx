...
interface ClassiListProps {
  onEdit: (Classification: ClassificationData) => void;
}

export default function ClassificationList({ onEdit }: ClassiListProps) {
  const [classifications, setclassification] = useState<ClassificationData[]>([]);
  ...
  const loadData = async () => {
    try {
      setLoading(true);
    const response = await classificationService.getclassification();
      ...
      {classifications.length === 0 ? (
        ...
        <div className="divide-y divide-gray-200">
          {classifications.map((classification) => (
            <div key={classification.id} className="p-6 hover:bg-gray-50 transition-colors">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                {/* المعلومات */}
                <div className="flex-1">
                  <p className="text-blue-600 font-medium">{classification.type}</p>
                </div>
